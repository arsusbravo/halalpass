        ...
        <div class="mx-auto max-w-3xl p-4">
            ...
            <div class="mb-6 rounded-xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-800 dark:bg-blue-950/30">
                ...
                <div class="grid gap-2 text-xs text-blue-700 dark:text-blue-300 sm:grid-cols-2">
                    ...
                    <div class="flex items-center gap-2"><svg v-if="form.team_members.length > 0 && form.team_members[0].name" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg><svg v-else class="h-3.5 w-3.5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg> 2. Tim Manajemen Halal<template v-if="!(form.team_members.length > 0 && form.team_members[0].name)"> — <strong>{{ t('fill in below') }}</strong></template></div>
                    <div class="flex items-center gap-2"><svg v-if="form.trainings.length > 0 && form.trainings[0].date" class="h-3.5 w-3.5 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg><svg v-else class="h-3.5 w-3.5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg> 3. Pelatihan dan Edukasi<template v-if="!(form.trainings.length > 0 && form.trainings[0].date)"> — <strong>{{ t('fill in below') }}</strong></template></div>
